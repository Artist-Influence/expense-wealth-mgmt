...
import { Search, Download, Check, CheckCheck, Edit3, X } from 'lucide-react';
...
export default function ReviewTable() {
  ...
  const saveEdit = async (tx: Transaction) => {
    ...
    if (!error) {
      ...
      await updateMerchantMemory(
        merchantKey,
        tx.mode as 'personal' | 'business',
        editValues.category,
        editValues.method || null,
        editValues.notes || null,
        tx.description_raw || '',
        user!.id
      );
      setEditingId(null);
  ...
  const approveRow = async (tx: Transaction) => {
    ...
    if (!category) {
      toast.error('Set a category before approving');
      return;
    }
  ...
  const exportCsv = () => {
    ...
    const headers = ['Date', 'Short Description', 'Total', 'Category', 'Method', 'Notes'];
    const csv = [
      headers.join(','),
                      ...
                      <td className="px-3 py-2">
                        {editingId === tx.id ? (
                          <Input
                            ...
                            className="glass-input h-7 text-xs w-32"
